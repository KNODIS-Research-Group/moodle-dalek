...
La instalación del _plugin_ en Moodle requiere seguir unos pasos
sencillos pero cruciales para asegurar su funcionamiento correcto. A
...
2. Verifica que la carpeta raíz del _plugin_ tenga un nombre distinto al
   ...
   coincide con otro plugin que tenas instalado, cámbialo para que
   coincida.
...
1. Si todo está correcto, Moodle instalará automáticamente las tablas de
...
1. Tras la instalación, accede a la configuración del plugin a través de
...
3. Comprueba que el cuestionario se genera correctamente y que las
   preguntas son coherentes.

### Paso 2: Habilitar la depuración (opcional)

1. Si encuentras errores, habilita el modo de depuración en Moodle para
...
Descarga la última versión del plugin desde el
[repositorio oficial]({{< var repository.base >}})

### Paso 2: Reemplazar los archivos

1. Realiza una copia de seguridad (nunca viene mal) de la carpeta del
...
Con estos pasos, el plugin {{< var plugin.name >}} estará completamente

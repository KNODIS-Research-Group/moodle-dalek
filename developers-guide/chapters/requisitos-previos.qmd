...
Moodle es el entorno en el que opera el plugin. Es imprescindible que la
...
de datos nos vale (e.g. MariaDB, Postgrsql). Aun así, comprueba el
manual de requisitos de Moodle para instalar el que mejor se ajuste a
tus necesidades.

### Cliente SQL

Para interactuar con la base de datos desde fuerea de Moodle (algo muy
útil en entornos de desarrollo), viene bien tener un cliente SQL como:
...
personalización o desarrollo del plugin.

Es importante conocer las bases del lenguaje PHP, ya que el código del
plugin está escrito en este lenguaje. También es interesante tener
conocimientos de cómo interactuar con API REST, ya que el plugin se
...
1. Una vez instalado, navega al directorio del plugin y ejecuta:
   ```bash
   composer install
   ```
